...
static Priority parsePriorityName(const std::string& s) {
...
static Season parseSeason(const std::string& s) {
...
static ShoppingItemFields readCommon(const JsonValue& s) {
    return ShoppingItemFields{
        s.at("name").asString(),
...
ShoppingItemFactory::ShoppingItemFactory() {
    _creators.emplace("Standard", [](const JsonValue& s) -> std::unique_ptr<ShoppingItem> {
        const ShoppingItemFields c = readCommon(s);
