...
namespace {
    // Cantitatea lipsa agregata pe ingredient, retinand si unitatea de masura.
    struct Shortage {
        Unit unit = Unit::g;
        double quantity = 0.0;
    };

    // Aduna deficitul (need - have) pentru ingredientele obligatorii ale retetelor recomandate.
    std::map<std::string, Shortage> aggregateShortages(const Inventory& inventory,
                                                       const std::vector<const Recipe*>& recipes,
                                                       int portions) {
        std::map<std::string, Shortage> shortages;
...
ShoppingList MealPlanner::planShoppingForTopRecipes(int topK, int portions,
    ...
    const std::map<std::string, Shortage> shortages =
    ...
    for (const auto& [name, shortage] : shortages) {
        ...
        // Construim specificatia produsului si lasam Factory-ul sa creeze obiectul.
...
void MealPlanner::exportPlan(const std::string& path, int topK, int portions,
    ...
    out << plan.dump(2);  // serializare cu nlohmann/json
}
